...
import { Leaflet, LeafletFile } from '@pharmaledgerassoc/ptp-toolkit/shared';
import { getNgxToastComponent } from 'src/app/utils/NgxToastComponent';
import { getDocumentTypes, getLeafletLanguages, getMarketOptions } from '../utils/helpers';
...
export class LeafletHandler extends NgxEventHandler {
  ...
  override async handle<Leaflet>(event: { data: Leaflet }): Promise<void> {
    ...
    if (success) {
      (this.router as Router).navigateByUrl('/leaflets', {
    ...
    if (xmlFileContent) {
      ...
      for (const file of arrayFiles) {
        ...
        if (isXmlFile) {
          ...
          data.otherFilesContent.push(fileContent as any);
        }
      }
    }
  ...
  static async getDocumentType(
    ...
    const { operation } = instance;
    if (!operation) {
      return getDocumentTypes().find((item) => value === item.value)?.text || '';
    }
    return 'subinfo';
  }
  ...
  static async getLanguage(instance: ListComponent, prop: string = 'lang', value: string): Promise<string> {
    const { operation } = instance;
    if (!operation) {
      return getLeafletLanguages().find((item) => value === item.value)?.text || '';
    }
    return 'subinfo';
  }

  static async getMarket(instance: ListComponent, prop: string = 'epiMarket', value: string): Promise<string | void> {
    return getMarketOptions().find((item) => value === item.value)?.text || '';
  }
...
function getLanguageOptions(): any {
