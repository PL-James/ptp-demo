...
import { Product, ProductImage } from '@pharmaledgerassoc/ptp-toolkit/shared';
...
export class ProductHandler<M extends Model> extends NgxEventHandler {
  ...
  static async getDiffs<M extends Model>(data: M, repository: IRepository<M>, modelId: PrimaryKeyType): Promise<any> {
      ...
      for (const key in data) {
        ...
        if (key in data) {
          ...
          if (item.other !== undefined) {
            (diff.other as Record<string, unknown>)[key] = item.other;
          }
          if (item.current !== undefined) {
            (diff.current as Record<string, unknown>)[key] = item.current;
          }
        }
      }
      const { other, current } = diff;
      if (!Object(other).length && !Object(current).length) {
        return undefined;
      }
      return diff;
    }
    ...
    for (const [key, value] of Object.entries(modelDiffs || {})) {
      ...
      const diff = value;
      const other = diff?.other || diff?.old || undefined;
      const current = diff?.current || diff?.new || undefined;
      if (Array.isArray(other)) {
        if (!other.length && current === undefined) continue;
      } else {
        if (Array.isArray(current)) {
