...
export class AppProductItemComponent
  ...
  implements OnInit
{
  override async ngOnInit() {
    ...
    this.item['batches'] = await this.getProductBatchesTotal(
  ...
  async getProductBatchesTotal(productCode: string): Promise<string> {
    ...
    if (repo) {
      ...
      const condition = Condition.attribute<Model>(
        'productCode' as keyof Model
      ).eq(productCode);
