...
import { CrudFieldComponent, Dynamic, ForAngularCommonModule, windowEventEmitter } from '@decaf-ts/for-angular';
import { ComponentEventNames } from '@decaf-ts/ui-decorators';
import { IonCheckbox, IonInput, IonItem, IonLabel, IonText } from '@ionic/angular/standalone';
import { TranslatePipe } from '@ngx-translate/core';
import { timer } from 'rxjs';
import { convertDateToGS1Format, formatExpiryDate, getLastDayOfMonth } from 'src/app/handlers/BatchHandler';
...
@Component({
  ...
  imports: [TranslatePipe, IonInput, IonCheckbox, IonItem, IonLabel, IonText, ForAngularCommonModule],
  standalone: true,
})
export class AppExpiryDateFieldComponent extends CrudFieldComponent implements OnInit {
  ...
  override async ngOnInit(): Promise<void> {
    ...
    if (this.operation === OperationKeys.CREATE) {
      ...
      if (this.name === 'enableDaySelection') {
        this.formGroup?.get(this.name)?.setValue(this.enableDaySelection);
      }
    }
  }
      ...
      if (source === 'enableDaySelection') {
        ...
        if (currentValue) {
          ...
          const value = this.enableDaySelection
            ? `${currentValue}-${getLastDayOfMonth(currentValue)}`
  ...
  parseCalendarInputValue(date: string, revert: boolean = false): string {
    if (revert) {
      return `${formatExpiryDate(date, this.enableDaySelection)}`;
    }
    return convertDateToGS1Format(date);
  }
  ...
  revertGS1ToDateValue(date: string): string {
    ...
    day = day === 0 ? Number(getLastDayOfMonth(`${year}-${month}`)) : day;
