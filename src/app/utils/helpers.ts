...
import { LeafletType } from '@pharmaledgerassoc/ptp-toolkit/shared';
...
export function getDocumentTypes(type: 'product' | 'batch' = 'product'): SelectOption[] {
  return Object.values(LeafletType)
    .map((doc) => ({
      text: doc.toLowerCase(),
      value: doc,
      disabled: doc === LeafletType.smpc,
      selected: doc === LeafletType.leaflet,
    }))
    .filter((doc) => {
      if (type === 'batch') return doc.value !== LeafletType.prescribingInfo;
      return doc;
    });
}
