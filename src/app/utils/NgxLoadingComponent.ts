...
export class NgxLoadingComponent extends LoggedClass {
  ...
  async update(
    message: string,
    isProgressUpdate: boolean | number = false
  ): Promise<void> {
  ...
  async remove(): Promise<void> {
    const loading: HTMLIonOverlayElement = this
      .instance as unknown as HTMLIonOverlayElement;
    if (!loading)
      return this.log
        .for(this.remove)
        .info(
          'Try Remove loading but no loading is present. Promise resolve will be called anyway'
        );
