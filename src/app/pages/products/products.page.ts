...
import { uionrender } from '@decaf-ts/ui-decorators';
...
import { ProductHandler } from 'src/app/handlers/ProductHandler';
import { ProductLayout } from 'src/app/layouts/ProductLayout';
import { EpiTabs } from 'src/app/utils';
...
@Component({
  ...
  providers: [AppProductItemComponent, AppSelectFieldComponent, AppModalDiffsComponent],
  imports: [
    IonContent,
    ModelRendererComponent,
...
export class ProductsPage extends NgxModelPageDirective implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    if (this.operation) {
      uionrender(() => ProductHandler)(Product, 'imageData');
    }
    ...
    //   (this.model as any).product = Model.build({ productCode: generateGtin() }, 'Product');
